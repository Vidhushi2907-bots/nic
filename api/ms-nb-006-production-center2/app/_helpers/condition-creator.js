...
class ConditionCreator {
    ...
    static breederBspFormFilter = async (filterData) => {
        let filter = {}
        for (var key in filterData) {
            var value = filterData[key];
            if (value) {
                switch (key) {
                    ...
                    case 'crop_code':
                        ...
                        break;
                    case 'crop_code_array':
                        filter = { crop_code: { [Op.in]: value }, ...filter }
                        break;
                    case 'variety_code':
                        filter = { variety_code: value, ...filter }
                        break;
                    case 'variety_line_code':
                        filter = { variety_line_code: value, ...filter }
                        break
                    case 'variety_id':
                        filter = { variety_id: value, ...filter }
                        break;
                    case 'crop_group':
                        filter = { crop_group: value, ...filter }
                        break;
                    case 'variety_code_array':
                        filter = { variety_code: { [Op.in]: value }, ...filter }
                        break;
                    case 'user_id':
                        filter = { user_id: value, ...filter }
                        break;
                    case 'bspc_id':
                        filter = { bspc_id: value, ...filter }
                        break;
                    case 'seed_class_id':
                        filter = { seed_class_id: value, ...filter }
                        break;
                    case 'meeting_number':
                        filter = { meeting_number: value, ...filter }
                        break;
    ...
    static bspcNewFlowFilter = async (filterData) => {
        let filter = {}
        for (var key in filterData) {
            var value = filterData[key];
            if (value) {
