...
class InabilityReallocates {
  ...
  static checkCaaryOverFinalSubmit = async (req, res) => {
    let filter = await ConditionCreator.bspcNewFlowFilter(req);
    let carryOverFinalSubmit = await db.bspProformaOneBspc.findAll({
      include: [
        {
          required: true,
          model: db.bspPerformaBspOne,
    ...
    try {
      ...
      let data = await db.inabilityReallocatesModel.findAll({
        include: [
          ...
          //   attributes: ['is_freezed'],
          //   where: {
          //     year: [sequelize.col("inability_reallocates.year")],
          //     season: [sequelize.col("inability_reallocates.season")],
          //     crop_code: [sequelize.col("inability_reallocates.crop_code")],
          //     variety_code: [sequelize.col("inability_reallocates.variety_code")],
          //     production_type: "REALLOCATION"
          //   }
          // }
        ],
      ...
      if (data && data.length) {
        for (let key of data) {
          ...
          let carryOverData = await this.checkCaaryOverFinalSubmit(param2);
          ...
          finalArray.push({
            ...
            season: key.season,
            crop_code: key.crop_code,
            is_freezed: carryOverData && carryOverData[0] && carryOverData[0]['carry_over_seed.is_freezed'] && carryOverData[0]['carry_over_seed.is_freezed'] ? (carryOverData[0]['carry_over_seed.is_freezed'] == 1 ? true : false) : false,
            variety_code: key.variety_code,
            ...
            if (bspcAssignTargetData && bspcAssignTargetData && bspcAssignTargetData.dataValues) {
              ...
              if (bspcData) {
                for (let key of bspc_reallocate_data) {
                  console.log('key.bspc_id===', key.bspc_id)
                  bspcReallocateData = await db.bspProformaOneBspc.create(
                    {
                      "bspc_proforma_1_id": bspcAssignTargetData.dataValues.id,
                      "bspc_id": key && key.bspc_id && key.bspc_id ? key.bspc_id : null,
                      // "bspc_id": key && key.bspc_id && key.bspc_id && key.bspc_id.bspc_id ? key.bspc_id.bspc_id:null,
        ...
        if (data) {
          ...
          if (bspc_reallocate_data && bspc_reallocate_data.length) {
            ...
            if (bspcAssignTargetData) {
              for (let key of bspc_reallocate_data) {
                // console.lokey.bspc_id(key.bspc_id);
                bspcReallocateData = await db.bspProformaOneBspc.create(
                  {
                    "bspc_proforma_1_id": bspcAssignTargetData.dataValues.id,
                    "bspc_id": key && key.bspc_id && key.bspc_id ? key.bspc_id : null,
  ...
  static deleteInabilityReallocatedData = async (req, res) => {
    try {
      ...
      if (getInabiltyData) {
        let varietylinecode;
        if (getInabiltyData.dataValues.variety_line_code) {
          varietylinecode = {
            variety_line_code: getInabiltyData.dataValues.variety_line_code
          }
        }
        let updateIscropFailre = await db.bspProrforma2Model.update({
          crop_failure: 0
        }, {
          where: {
            ...
            ...varietylinecode,
            user_id: req.body.loginedUserid?.id || ''  // Optional chaining and fallback to '' if undefined
          }
        });
  ...
  static getAllWillingBspcListData = async (req, res) => {
    try {
      ...
      console.log('getAllBspcList========', getAllBspcList);
      let finalArray = []
      if (getAllBspcList && getAllBspcList.length) {
        for (let key of getAllBspcList) {
          ...
          finalArray.push({
            ...
            "nucleus_seed_available": nucleusSeedData && nucleusSeedData[0] && nucleusSeedData[0].nucleus_seed_available ? (req.body && req.body.search && req.body.search.crop_code.slice(0, 1) == "A" ? (nucleusSeedData[0].nucleus_seed_available / 100) : nucleusSeedData[0].nucleus_seed_available) : 0,
            "breeder_seed_available": breederSeedData && breederSeedData[0] && breederSeedData[0].breeder_seed_available ? (req.body && req.body.search && req.body.search.crop_code.slice(0, 1) == "A" ? (breederSeedData[0].breeder_seed_available / 100) : breederSeedData[0].breeder_seed_available) : 0
          })
        }
