...
class IndentorController {
  ...
  static saveDirectIndent = async (req, res) => {
    try {
      ...
      let isSelfValue;
      if (is_self) {
        agencyData = await userModel.findOne({
          include: [
            {
          ...
          attributes: [
            ...
            [sequelize.col('agency_detail.district_id'), 'district_id'],
            [sequelize.col('agency_detail.address'), 'address'],
            [sequelize.col('agency_detail.mobile_number'), 'mobile_number'],
            [sequelize.col('agency_detail.email'), 'email'],
            [sequelize.col('user.id'), 'user_id'],
            [sequelize.col('user.name'), 'name'],
            [sequelize.col('user.mobile_number'), 'mobile'],
            [sequelize.col('user.email_id'), 'email_id'],
            [sequelize.col('user.username'), 'username'],
          ],
          where: {
            id: req.body.loginedUserid.id
          },
          raw: true
        });
        isSelfValue = {
      ...
      const newDirectIndent = await directIndentModel.create({
        ...
        state_code: state_code ? state_code : agencyData && agencyData.state_id ? agencyData.state_id : null,
        district_code: district_code ? district_code : agencyData && agencyData.district_id ? agencyData.district_id : null,
        spa_name: spa_name ? spa_name : agencyData && agencyData.agency_name ? agencyData.agency_name : null,
        spa_id: spa_id ? spa_id : 0,
        spa_address: spa_address ? spa_address : agencyData && agencyData.address ? agencyData.address : null,
        spa_mobile_number: spa_mobile_number ? spa_mobile_number : agencyData && agencyData.mobile ? agencyData.mobile : null,
        quantity: quantity,
        is_active: is_active,
        user_id: user_id,
        email_id: email_id ? email_id : agencyData && agencyData.email_id ? agencyData.email_id : null,
        ...isSelfValue
      });
      let totalQuantity = 0;
  ...
  static getAllDirectIndents = async (req, res) => {
    try {
      const { page, pageSize, crop, year, variety, season, user_id, is_self } = req.body;
      ...
      // }
      if (is_self === true) {
        filterOptions.where.is_self = 1;
      } else {
        filterOptions.where.is_self = 0;
      }
  ...
  static getBspProformaOneDataDynamic = async (req, res) => {
    let data;
    if (req) {
      data = await bspProforma1sModel.findOne({
        include: [
          {
  ...
  static getBspProforma3DataReport = async (req, res) => {
    try {
        ...
        include: [
          {
            ...
            include: [
              ...
              //   crop_code: sequelize.col('bspProforma2.crop_code'),
              //   year: sequelize.col('bspProforma2.year'),
              //   season: sequelize.col('bspProforma2.season'),
              //   variety_code: sequelize.col('bspProforma2.variety_code'),
              //   attributes: ['id', 'created_at'],
              //   include: [
              //     {
              //       model: bspProforma1BspcsModel,
              //       where:{
              //         bspc_id: sequelize.col('bspProforma2.user_id')
              //       },
              //       attributes:[]
              //     }
              //   ]
              //   // model: bspProforma1BspcsModel,
              //   // attributes: [sequelize.fn("DISTINCT", sequelize.col('bspProforma1BspcsModel.bspc_id')), 'bspc_proforma_1_id'],
      ...
      if (responseData && responseData.length) {
        for (const [i, key] of responseData.entries()) {
          let param = {
            year: key.bspProforma2.year,
            crop_code: key.bspProforma2.crop_code,
            season: key.bspProforma2.season,
            variety_code: key.bspProforma2.variety_code,
            user_id: key.bspProforma2.user_id
          }
          let bspOneData = await this.getBspProformaOneDataDynamic(param)
          if (bspOneData) {
            key.bspProforma2.created_at_bsp1 = bspOneData.created_at;
          } else {
            key.bspProforma2.created_at_bsp1 = null;
          }
        }
      }
  ...
  static getBspProforma3Data = async (req, res) => {
    try {
      ...
      const bspProforma3Data = await bspProrforma3Model.findAndCountAll({
        where: whereClause,
        limit: pageSize,
        offset: (page - 1) * pageSize,
        include: [
      ...
      console.log('bspProforma3Data.rows[i]===', bspProforma3Data.rows)
      const agencyDetails = await agencyDetailModel.findOne({
        where: { user_id: user_id },
        include: [
          {
      ...
      const responseData = bspProforma3Data.rows.map(item => {
        return {
          ...item.toJSON(),
          agencyDetails: {
            ...agencyDetails.toJSON()
  ...
  static createNewSpa = async (req, res) => {
    try {
            ...
            [Op.and]: [
              {
                where: sequelize.where(
                  sequelize.fn('lower', sequelize.col('agency_name')),
                  sequelize.fn('lower', agencyData.agency_name),
                ),
                // created_by: { [Op.eq]: req.body.created_by }
              },
            ]
            ...
            [Op.and]: [
              {
                where: sequelize.where(
                  sequelize.fn('lower', sequelize.col('email')),
                  sequelize.fn('lower', agencyData.email_id),
                ),
              },
            ]
          },
      ...
      const userData = await userModel.create({
        name: spa_name ? spa_name : agencyData && agencyData.agency_name ? agencyData.agency_name : null,
        username: email_id ? email_id : agencyData && agencyData.email_id ? 'spa_' + agencyData.email_id : null,
        password: '123456',
        user_type: 'SPA',
        is_active: 1,
        mobile_number: mobile_number ? mobile_number : agencyData && agencyData.mobile ? agencyData.mobile : null,
        email_id: email_id ? email_id : agencyData && agencyData.email_id ? agencyData.email_id : null,
        unm: email_id ? email_id : agencyData && agencyData.email_id ? agencyData.email_id : null,
        spa_code: email_id ? email_id : agencyData && agencyData.email_id ? agencyData.email_id : null,
      });
      let agencydataValue;
      if (userData) {
        agencydataValue = await agencyDetailModel.create({
          user_id: userData.id,
          agency_name: spa_name ? spa_name : agencyData && agencyData.agency_name ? agencyData.agency_name : null,
          state_id: state_id ? state_id : agencyData && agencyData.state_id ? agencyData.state_id : null,
          district_id: district_id ? district_id : agencyData && agencyData.district_id ? agencyData.district_id : null,
          address: address ? address : agencyData && agencyData.address ? agencyData.address : null,
          mobile_number: mobile_number ? mobile_number : agencyData && agencyData.mobile ? agencyData.mobile : null,
          email: email_id ? email_id : agencyData && agencyData.email_id ? 'spa_' + agencyData.email_id : null,
          phone_number: mobile_number ? mobile_number : agencyData && agencyData.mobile ? agencyData.mobile : null,
          is_active: 1,
          spa_name: spa_name ? spa_name : agencyData && agencyData.agency_name ? agencyData.agency_name : null
        });
        if (agencydataValue) {
          await userModel.update(
            { agency_id: agencydataValue.id },
  ...
  static checkExistingSpa = async (req, res) => {
    try {
      let { state_code, district_code, spa_address, year, season, variety_code, crop_code, spa_mobile_number } = req.body;

      // Convert empty values to null
      state_code = state_code ? parseInt(state_code, 10) : null;
      district_code = district_code ? parseInt(district_code, 10) : null;
      spa_mobile_number = spa_mobile_number && spa_mobile_number.trim() !== "" ? spa_mobile_number : null;


      // Query to check for existing record
      const existingRecord = await directIndentModel.findOne({
        where: {
          state_code: state_code !== null ? state_code : { [Op.ne]: null },  // Avoid empty integer fields
          district_code: district_code !== null ? district_code : { [Op.ne]: null },
          spa_address,
          year,
          season,
          variety_code: variety_code !== null ? variety_code : { [Op.ne]: null },
          crop_code,
          spa_mobile_number: spa_mobile_number !== null ? spa_mobile_number : { [Op.ne]: null }
        }
      });

      if (existingRecord) {
        return res.status(200).json({ status: 200, exists: true });
      }
      return res.status(200).json({ status: 200, exists: false });
    } catch (error) {
    ...
    try {
      ...
      const bspProforma3Data = await bspProrforma3Model.findAll({
        ...
        include: [
          {
            ...
            include: [
              ...
              //       model: db.bspPerformaBspOne,
              //       where: {
              //         year: [sequelize.col('bsp_proforma_3s.year')],
              //         season: [sequelize.col('bsp_proforma_3s.season')],
              //         crop_code: [sequelize.col('bsp_proforma_3s.crop_code')],
              //         variety_code: [sequelize.col('bsp_proforma_3s.variety_code')]
              //       },
              //       attributes: [],
              //     }
              //   ]
        ...
        attributes: [
          ...
          // [sequelize.col('bspProforma2->bsp_proforma_1_bspc.target_qunatity'), 'bspc_assigin_target'],
          [sequelize.col('bspProforma2->indent_of_breederseed_direct.quantity'), 'indent_direct_target'],

          // temp code
          [sequelize.col('bsp_proforma_1->bsp_proforma_1_bspc.target_qunatity'), 'bspc_assigin_target'],
      ...
      const groupedData = Object.values(bspProforma3Data.reduce((acc, item) => {
        // Check if the field_code already exists in accumulator
        if (!acc[item.field_code]) {
          // If not, initialize with the first object's values
          acc[item.field_code] = { ...item, indent_direct_target: 0 };
        }
        // Add the indent_direct_target to the existing one
        acc[item.field_code].indent_direct_target += item.indent_direct_target;
        return acc;
      }, {}));

      if (groupedData && groupedData.length) {
        const updatedData = groupedData.map(item => {
          const bspcAssiginTarget = item.bspc_assigin_target || 0; // If it's null, treat it as 0
          const indentDirectTarget = item.indent_direct_target || 0; // If it's null, treat it as 0
          const targetQnt = bspcAssiginTarget + indentDirectTarget; // Sum the targets
          return {
            ...item,  // Copy all other properties
            target_qnt: targetQnt,  // Add the new target_qnt key
          };
        });
        return response(res, status.DATA_NOT_AVAILABLE, 200, updatedData);
      } else {
  ...
  static addInvestingVerify = async (req, res) => {
    try {
      ...
      console.log('investingResponse====', investingResponse);
  ...
  static getLotNo = async (req, res) => {
    try {
      ...
      console.log(investHarvesting, 'investHarvesting')
      if (investHarvesting) {
        return response(res, status.DATA_AVAILABLE, 200, investHarvesting ? investHarvesting : 0)
      } else {
        return response(res, status.DATA_AVAILABLE, 200, 0)
      }
    } catch (error) {
      console.error('Error fetching data:', error);
      return response(res, status.DATA_NOT_AVAILABLE, 500, error)
  ...
  static getBspProformaOneYearDataReport = async (req, res) => {
    try {
      ...
      if (req.body.search && req.body.search.form_type == 'report_1') {
        ...
        if (req.body.production_type) {
          if (req.body.production_type == "DELAY") {
            production_type = { production_type: "DELAY" }
          }
          if (req.body.production_type == "NORMAL") {
            production_type = { production_type: "NORMAL" }
          }
          if (req.body.production_type == "REALLOCATION") {
  ...
  static getBspOnePerformaYearDataReport = async (req, res) => {
    try {
      ...
      if (req.body.search && req.body.search.form_type == 'report_2') {
        ...
        if (req.body.search.production_type) {
          if (req.body.search.production_type == "DELAY") {
            production_type = { production_type: "DELAY" }
          }
          if (req.body.search.production_type == "NORMAL") {
            production_type = { production_type: "NORMAL" }
          }
          if (req.body.search.production_type == "REALLOCATION") {
            production_type = { production_type: "REALLOCATION" }
          }
        }
      }
  ...
  static getBspProformaYearsReport = async (req, res) => {
    try {
      ...
      let bspProforma2Data = await bspProrforma2Model.findAll({
        attributes: ['id', 'year', 'season', 'crop_code', 'user_id', 'is_inspected'],
        where: {
          ...production_type,
          // ...is_inspected,
          ...cropGroup
      ...
      let filteredData;
      if (req.body && req.body.report_type == 'intake-report') {
        const grouped = bspProforma2Data.reduce((acc, record) => {
          const key = `${record.year}-${record.season}-${record.crop_code}-${record.user_id}`;
          if (!acc[key]) acc[key] = [];
          acc[key].push(record);
          return acc;
        }, {});

        // Step 2: Filter out groups where any record has is_inspected as false
        filteredData = Object.values(grouped)
          .filter(group => group.every(record => record.is_inspected === true))
          .flat(); // Flatten the array back into a single array
      } else {
        filteredData = bspProforma2Data;
      }
      const bspProforma2IDs = filteredData.map((data) => data.id);
  ...
  static CropStatusReport = async (req, res) => {
    try {
      ...
      if (req.body.search.crop_type) {
        if (req.body.search.crop_type == "horticulture") {
          crop_type = { crop_code: { [Op.like]: 'H' + '%' } }
        }
        if (req.body.search.crop_type == "agriculture") {
          crop_type = { crop_code: { [Op.like]: 'A' + '%' } }
        }
      }
      ...
      let condition = {
        ...
        attributes: [
          ...
          [sequelize.col('m_crop->user->agency_detail.agency_name'), 'pdpc_name'],
          [sequelize.col('m_crop.breeder_id'), 'pdpc_id'],
      ...
      if (data && data.length) {
        ...
        for (let key of data) {
          ...
          if (key.crop_code && !key.user_id) {
            final_map[key.crop_code] = {
              user_id: key.user_id,
              crop_type: cropType,
              Unit: unitKgQ,
              crop_code: key.crop_code,
              ...
              bsp_1_status: 'Pending',
              bspc_status: [{
                "bspc_id": key.bspc_id,
                "bspc_name": key.bspc_name,
                "bsp_2_status": 'Pending',
                "bsp_3_status": 'Pending'
              }]
            }
            final_map[key.crop_code].variety_codes.push(key.variety_code);
            final_map[key.crop_code].indent_quantity.push(key.indent_quantity);
          ...
          // Checks if final_map already contains an entry for crop_code.
          // If it does NOT exist, a new entry is created with default values.
          // If it already exists, it skips creating a new entry (to avoid duplicates).
  ...
  static getIntakeVerificationStatus = async (req, res) => {
    let checkStatusData = await db.investHarvestingModel.findOne(
      {
        where: {
          year: req.year,
          season: req.season,
        ...
        attributes: ['check_status', 'actual_harvest_date', 'variety_code', 'crop_code', 'user_id', 'year'],
        raw: true
      },
  ...
  static getIntakeVerificationRegisterStatusReports = async (req, res) => {
    try {
      let rules = {
        "year": 'required',
        "season": 'required|string',
      ...
      let condition = {
        ...
        attributes: ['id',
          // [sequelize.fn("DISTINCT",sequelize.col('bsp_proforma_2s.id')), 'id'],
          [sequelize.col('bsp_proforma_2s.user_id'), 'user_id'],
          [sequelize.col('bsp_proforma_2s.year'), 'year'],
          [sequelize.col('bsp_proforma_2s.season'), 'season'],
          [sequelize.col('bsp_proforma_2s.crop_code'), 'crop_code'],
          [sequelize.col('bsp_proforma_2s.variety_code'), 'variety_code'],
          [sequelize.col('m_crop.crop_name'), 'crop_name'],
          [sequelize.col('bsp_proforma_2s.is_inspected'), 'is_inspected'],
          // [sequelize.col('invest_harvesting.check_status'), 'check_status'],
          // [sequelize.col('invest_harvesting.actual_harvest_date'), 'actual_harvest_date'],
          [sequelize.col('user->agency_detail.agency_name'), 'agency_name'],
          [sequelize.col('user.code'), 'bspc_code'],
        ],
        where: {
          ...production_type,
          ...cropGroup
        ...
        raw: true,
        order: [[sequelize.col('bsp_proforma_2s.user_id'), 'ASC']]
      }
      if (req.body) {
        if (req.body.search) {
          if (req.body.search.year) {
            condition.where.year = req.body.search.year
            // condition.include[0].where.year = req.body.search.year
          }
          if (req.body.search.season) {
            condition.where.season = req.body.search.season
            // condition.include[0].where.season = req.body.search.season
          }
          if (req.body.search.crop_code) {
            condition.where.crop_code = req.body.search.crop_code
            // condition.include[0].where.crop_code = req.body.search.crop_code
          }
          if (req.body.search.variety_code) {
            condition.where.variety_code = req.body.search.variety_code
            // condition.include[0].where.variety_code = req.body.search.variety_code
          }
          if (req.body.search.variety_code_array && req.body.search.variety_code_array.length) {
            condition.where.variety_code = {
              [Op.in]: req.body.search.variety_code_array
      ...
      let data = await db.bspPerformaBspTwo.findAll(condition);
      // Function to remove groups where any record has `is_inspected: false`
      let filterData = (data) => {
        // Group the data by year, season, crop_code, and user_id
        const grouped = data.reduce((acc, item) => {
          const key = `${item.year}-${item.season}-${item.crop_code}-${item.user_id}`;
          if (!acc[key]) {
            acc[key] = [];
          }
          acc[key].push(item);
        ...
        // Filter out the groups where any record has `is_inspected: false`
        const validGroups = Object.values(grouped).filter(group => {
          // If any record in this group has is_inspected === false, we exclude the group
          return !group.some(item => item.is_inspected === false);
        });

        // Flatten the valid groups back into a single array
        return validGroups.flat();
      };
      // Example usage
      let filteredData = filterData(data);
      let checkStatusArray = []
      for (let [i, key] of filteredData.entries()) {
        let statusData = await this.getIntakeVerificationStatus({
          year: key.year,
          season: key.season,
          crop_code: key.crop_code,
        ...
        let status;
        if (statusData && statusData.check_status == true) {
          status = "true"
        } else if (statusData && statusData.check_status == false) {
          status = "false"
        } else {
          status = null
        }
        filteredData[i].check_status = status//statusData.check_status ? statusData.check_status : nill;
        filteredData[i].actual_harvest_date = statusData && statusData.actual_harvest_date ? statusData.actual_harvest_date : null
      }
      console.log('filteredData===========', filteredData);
      if (filteredData && filteredData.length) {
        function groupAndDetermineStatus(filteredData) {
          // Group by year, season, crop, and user_id
          const groupedData = filteredData.reduce((result, item) => {
            const key = `${item.year}-${item.season}-${item.crop_code}-${item.user_id}`;
            if (!result[key]) {
              result[key] = {
                year: item && item.year ? item.year : null,
                season: item && item.season ? item.season : null,
                crop_name: item && item.crop_name ? item.crop_name : null,
                crop_code: item && item.crop_code ? item.crop_code : null,
                user_id: item && item.user_id ? item.user_id : null,
                name: item && item.agency_name ? item.agency_name : null,
                code: item && item.bspc_code ? item.bspc_code : null,
                submit_date: item && item.actual_harvest_date ? item.actual_harvest_date : null,
                check_status: []
              };
            }
            result[key].check_status.push(item.check_status);
            return result;
          }, {});

          // Transform grouped data to include the status
          return Object.values(groupedData).map(group => {
            const { check_status } = group;
            let status = '';
            if (check_status.every(status => status === null || status === 'null' || status === '')) {
              status = 'Pending';
            } else if (check_status.every(status => status === "true" || status === "false")) {
              status = 'Completed';
            } else {
              status = 'Working';
            }
        ...
        const groupedAndStatusData = groupAndDetermineStatus(filteredData);
        return response(res, status.DATA_AVAILABLE, 200, groupedAndStatusData);
      }
      else {
        return response(res, status.DATA_NOT_AVAILABLE, 201, [])
      }
