...
const bsp_proforma_1Model = require('../models/bsp_proforma_1.model');
...
class ProcessedSeedDetails {
  ...
  static getYearofBsp4 = async (req, res) => {
    ...
    try {
      ...
      let data3 = await db.bspPerformaBspTwo.findAll({
        attributes: [
          [db.Sequelize.fn("Distinct", db.Sequelize.col("bsp_proforma_2s.year")), "value"]
        ],
        where: {
          user_id: req.body.loginedUserid.id
        },
      ...
      let data4 = await db.bspPerformaBspThree.findAll({
        include: [
          {
        ...
        attributes: [
          [db.Sequelize.fn("Distinct", db.Sequelize.col("bsp_proforma_3s.year")), "value"],
        ],
        where: {
        },
        raw: true
  ...
  static getSeeasonofBspFour = async (req, res) => {
    ...
    try {
      ...
      let data3 = await db.bspPerformaBspTwo.findAll({
        attributes: [
          [db.Sequelize.fn("Distinct", db.Sequelize.col("bsp_proforma_2s.season")), "value"]
        ],
        where: {
          year: req.body && req.body.search.year,
          user_id: req.body.loginedUserid.id
        },
        raw: true
      });
      ...
      let data4 = await db.bspPerformaBspThree.findAll({
        ...
        attributes: [
          [db.Sequelize.fn("Distinct", db.Sequelize.col("bsp_proforma_3s.season")), "value"],
        ],
        where: {
          year: req.body && req.body.search.year,
        },
  ...
  static getCropofBspFour = async (req, res) => {
    ...
    try {
      ...
      let data3 = await db.bspPerformaBspTwo.findAll({
        attributes: [
          [db.Sequelize.fn("Distinct", db.Sequelize.col("bsp_proforma_2s.crop_code")), "value"]
        ],
        where: {
          year: req.body && req.body.search.year,
          season: req.body && req.body.search.season,
          user_id: req.body.loginedUserid.id
        },
        raw: true
      });
      ...
      let data4 = await db.bspPerformaBspThree.findAll({
        ...
        attributes: [
          [db.Sequelize.fn("Distinct", db.Sequelize.col("bsp_proforma_3s.crop_code")), "value"],
        ],
        where: {
          year: req.body && req.body.search.year,
          season: req.body && req.body.search.season,
        ...
        for (let key in bspc) {
          if (bspc && bspc[key] && bspc[key].available_id && bspc[key].available_id) {
            const dataRow = {
              ...
              // new code 
              bsp2_qty: bspc && bspc[key] && bspc[key].bsp2_qty ? bspc[key].bsp2_qty : null,
              bsp2_per_qty: bspc && bspc[key] && bspc[key].bsp2_per_qty ? bspc[key].bsp2_per_qty : null,
              bsp3_qty: bspc && bspc[key] && bspc[key].bsp3_qty ? bspc[key].bsp3_qty : null,
              bsp3_per_qty: bspc && bspc[key] && bspc[key].bsp3_per_qty ? bspc[key].bsp3_per_qty : null,
              intake_vrfictn_qty: bspc && bspc[key] && bspc[key].intake_vrfictn_qty ? bspc[key].intake_vrfictn_qty : null,
              intake_vrfictn__pr_qty: bspc && bspc[key] && bspc[key].intake_vrfictn__pr_qty ? bspc[key].intake_vrfictn__pr_qty : null,
              check_status: bspc && bspc[key] && bspc[key].check_letest ? bspc[key].check_letest : null,
  ...
  static getYearofTagsNumber = async (req, res) => {
    ...
    try {
      ...
      let seedProcessRegData = await db.seedProcessingRegister.findAll({
        attributes: [
          [db.Sequelize.fn("Distinct", db.Sequelize.col("seed_processing_register.year")), "value"],
        ],
        where: {
          crop_code: 'H1101',
          bspc_id: req.body.loginedUserid.id
      ...
      // Create a Set for unique values from both arrays
      const uniqueValuesSet = new Set([...data, ...seedProcessRegData].map(item => item.value));

      // Create the union array based on unique values
      const unionData = [...uniqueValuesSet].map(value => ({ value }));

      if (unionData && unionData.length) {
        return response(res, status.DATA_AVAILABLE, 200, unionData)
      } else {
        return response(res, "Data Not Found", 200, {})
      }
  ...
  static getSeasonofTagsNumber = async (req, res) => {
    ...
    try {
      ...
      let seedProcessRegData = await db.seedProcessingRegister.findAll({
        attributes: [
          [db.Sequelize.fn("Distinct", db.Sequelize.col("seed_processing_register.season")), "value"],
        ],
        where: {
          crop_code: 'H1101',
          bspc_id: req.body.loginedUserid.id
  ...
  static getCropofTagsNumber = async (req, res) => {
    ...
    try {
      ...
      let seedProcessRegData = await db.seedProcessingRegister.findAll({
        ...
        attributes: [
          [db.Sequelize.fn("Distinct", db.Sequelize.col("seed_processing_register.crop_code")), "crop_code"],
          [sequelize.col('m_crop.crop_name'), 'crop_name'],
        ],
        where: {
          crop_code: 'H1101',
      ...
      const uniqueCropCodesSet = new Set([
        ...data.map(item => item.crop_code),
        ...seedProcessRegData.map(item => item.crop_code)
      ]);

      // Create the union array based on unique crop_codes
      const unionData = [
        ...data.filter(item => uniqueCropCodesSet.has(item.crop_code)),
        ...seedProcessRegData.filter(item => uniqueCropCodesSet.has(item.crop_code))
      ];
      if (unionData && unionData.length) {
        return response(res, status.DATA_AVAILABLE, 200, unionData)
      } else {
    ...
    try {
      ...
      const condition = {
        ...
        attributes: [
          ...
          // [sequelize.col('m_crop.crop_name'), 'crop_name'],
          [sequelize.col('seed_processing_register.lot_id'), 'lot_id'],
          [sequelize.col('seed_processing_register.id'), 'spr_id'],
          [sequelize.col('seed_processing_register.is_generate'), 'is_generate'],
          [sequelize.col('seed_processing_register.lot_no'), 'lot_no'],
      ...
      let seedProcessRegData = await db.seedProcessingRegister.findAll({
        ...
        attributes: [
          ...
          [sequelize.col('seed_processing_register.action'), 'action'],
          [sequelize.col('seed_processing_register.total_processed_qty'), 'total_processed_qty'],
          [sequelize.col('seed_for_processed_stack.no_of_bag'), 'no_of_bag_stack'],
          [sequelize.col('seed_for_processed_stack.stack_no'), 'stack_no_stack'],
          [sequelize.col('seed_for_processed_stack.godown_no'), 'godown_no_stack'],
          // [sequelize.col('seed_processing_register->processed_seed_detail.bag_size'), 'bag_size_stack'],
          [sequelize.col('seed_processing_register.lot_qty'), 'lot_qty'],
          [sequelize.col('seed_processing_register.no_of_bags'), 'no_of_bags'],
          [sequelize.col('seed_processing_register.godown_no'), 'godown_no'],
          [sequelize.col('seed_processing_register.stack_no'), 'stack_no'],
          [sequelize.col('seed_processing_register.id'), 'seed_register_id'],
          [sequelize.col('processed_seed_detail.bag_size'), 'bag_size_processed'],
          [sequelize.col('processed_seed_detail.no_of_bags'), 'no_of_bags_processed'],
          [sequelize.col('processed_seed_detail.qty'), 'qty_procecessed'],
          [sequelize.col('processed_seed_detail.id'), 'proceseed_id'],
      ...
      let mergedArray;
      if (req.body && req.body.search && req.body.search.crop_code == "H1101") {
        // const mergedArray = data.concat(seedProcessRegData);
        mergedArray = seedProcessRegData
      } else {
        mergedArray = data;
      }
      ...
      if (mergedArray && mergedArray.length > 0) {
        let caaryOverData;
        mergedArray.forEach(async (el, index) => {
          let varietyIndex;
          ...
          if (varietyIndex == -1) {
            filterData.push({
              ...
              lot_details: [
                {
                  spr_id: el.spr_id ? el.spr_id : null,
                  is_generate: el.is_generate ? el.is_generate : null,
  ...
  static isValidDate(dateStr, format) {
    const regexPatterns = {
    ...
    return regexPatterns[format].test(dateStr);
  }
  ...
  static savegeneratingLotTags = async (req, res) => {
    try {
      const {
        ...
        code, class_of_seed,
        valid_upto, user_id, bagsData, spr_id
      } = req.body;
      let seedTag;
      let timestamp;
      let localDate;
      ...
      let temp;
      switch (true) {
        case this.isValidDate(date_of_test, "YYYY-MM-DD HH:mm"):
          temp = date_of_test.split(" ")[0];
          [day, month, years] = temp.split("-").map(Number);
          localDate = new Date(years, month - 1, day, 0, 0, 0);
          timestamp = localDate.toISOString().slice(0, 10)
          date = moment().subtract(0, 'days');
          date.format('YYYY-MM-DD H:mm:ss');
          // return "Format: YYYY-MM-DD HH:MM";
          break;
        case this.isValidDate(date_of_test, "DD-MM-YYYY"):
          [day, month, years] = date_of_test.split("-").map(Number);
          localDate = new Date(years, month - 1, day, 0, 0, 0);
          timestamp = localDate.toISOString().slice(0, 10)
          date = moment().subtract(0, 'days');
          date.format('YYYY-MM-DD H:mm:ss');
          ...
          break;
        case this.isValidDate(date_of_test, "YYYY/MM/DD"):
        ...
        case this.isValidDate(date_of_test, "YYYY-MM-DD"):
          ...
          // return "Format: YYYY-MM-DD";
          break;
        case this.isValidDate(date_of_test, "DD/MM/YYYY HH:mm"):
          temp = date_of_test.split(" ")[0];
          [day, month, years] = temp.split("/").map(Number);
          localDate = new Date(years, month - 1, day, 0, 0, 0);
          timestamp = localDate.toISOString().slice(0, 10)
          ...
          break;
        case this.isValidDate(date_of_test, "DD/MM/YYYY HH:mm:ss"):
          temp = date_of_test.split(" ")[0];
          [day, month, years] = temp.split("/").map(Number);
          localDate = new Date(years, month - 1, day, 0, 0, 0);
          timestamp = localDate.toISOString().slice(0, 10)
      ...
      if (spr_id) {
        await db.seedProcessingRegister.update({
          is_generate: 1
        }, {
          where: {
            id: spr_id
    ...
    try {
      ...
      let condition = {
        where: {
          ...
          [Op.or]: [
            { is_discard: { [Op.not]: 1 } },
            { is_discard: null } // Ensures null values are included
          ]
        }
      }
  ...
  static getVarietyDataofStl = async (req, res) => {
    try {
      ...
      let seedProcessRegData = await db.seedProcessingRegister.findAll({
        ...
        attributes: [
          [db.Sequelize.fn("Distinct", db.Sequelize.col("seed_processing_register.variety_code")), "value"],
          [sequelize.col('m_crop_variety.variety_name'), 'display_text'],
        ],
        where: {
          crop_code: 'H1101',
      ...
      const uniqueCropCodesSet = new Set([
        ...
        ...seedProcessRegData.map(item => item.value)
      ]);

      // Create the union array based on unique crop_codes
      const unionData = [
        ...data.filter(item => uniqueCropCodesSet.has(item.value)),
        ...seedProcessRegData.filter(item => uniqueCropCodesSet.has(item.value))
      ];
      if (unionData && unionData.length) {
        return response(res, status.DATA_AVAILABLE, 200, unionData)
      } else {
  ...
  static getLotDataofStl = async (req, res) => {
    try {
      ...
      let seedProcessRegData = await db.seedProcessingRegister.findAll({
        attributes: [
          [db.Sequelize.fn("Distinct", db.Sequelize.col("seed_processing_register.lot_id")), "value"],
          [db.Sequelize.col("seed_processing_register.lot_no"), "display_text"],
        ],
        where: {
          crop_code: 'H1101',
          bspc_id: req.body.loginedUserid.id
  ...
  static getbspfourData = async (req, res) => {
    ...
    try {
      ...
      const condition = {
        ...
        include: [
...

          {
            ...
            include: [
              ...
              {
                required: false,
                model: db.availabilityOfBreederSeedModel,
                attributes: [],
                where: {
                  ...
                  variety_code: [sequelize.col('bsp_proforma_1s.variety_code')],
                },
                include: [
                  {
                    required: false,
        ...
        attributes: [
          // [db.Sequelize.fn("DISTINCT", db.Sequelize.col("bsp_proforma_1_bspc->availability_of_breeder_seed.crop_code")), "value"], 
          [db.Sequelize.fn("DISTINCT", db.Sequelize.col("bsp_proforma_1_bspc->user->agency_detail.agency_name")), 'user_name'],
          [db.Sequelize.col("bsp_proforma_1s.crop_code"), 'crop_code'],
          [db.Sequelize.col("m_crop_variety.id"), 'variety_id'],
          [db.Sequelize.col("m_crop_variety.variety_name"), 'variety_name'],
          [db.Sequelize.literal('SUM("bsp_proforma_1_bspc->availability_of_breeder_seed"."breeder_see_qty")'), 'breeder_see_qty'],
          [db.Sequelize.literal('SUM("bsp_proforma_1_bspc->availability_of_breeder_seed"."total_breeder_seed_qty")'), 'total_breeder_seed_qty'],
          [db.Sequelize.literal('SUM("bsp_proforma_1_bspc"."target_qunatity")'), 'target_qty_national'],
          [db.Sequelize.literal('SUM("bsp_proforma_1_bspc->availability_of_breeder_seed"."allocate_qty")'), 'allocate_qty'],
          [db.Sequelize.col("bsp_proforma_1_bspc.bspc_id"), 'user_id'],
          [db.Sequelize.col("bsp_proforma_1s.variety_code"), 'variety_code'],
          [db.Sequelize.col("bsp_proforma_1_bspc->availability_of_breeder_seed->m_crop.crop_name"), 'crop_name'],
          [db.Sequelize.col("bsp_proforma_1s.variety_line_code"), 'variety_code_line'],
          [db.Sequelize.col("bsp_proforma_1_bspc->availability_of_breeder_seed->m_variety_line.line_variety_name"), 'line_variety_name'],

        ],
        group: [
          'bsp_proforma_1s.id', // Grouping based on the main table's ID to prevent aggregation errors
          "bsp_proforma_1_bspc.bspc_id",
          "m_crop_variety.id",
          'm_crop_variety.variety_name',
          'bsp_proforma_1_bspc->user->agency_detail.agency_name',
          'bsp_proforma_1s.crop_code',
          'bsp_proforma_1s.variety_code',
          'bsp_proforma_1s.variety_line_code',
          'bsp_proforma_1_bspc->availability_of_breeder_seed->m_crop.crop_name',
          'bsp_proforma_1_bspc->availability_of_breeder_seed->m_variety_line.line_variety_name'
        ],
      ...
      //  condition.order[['year','ASC']]
      let data = await db.bspPerformaBspOne.findAll(condition)
  ...
  //   }
  // }
  static getLiftingDataViewPrint1 = async (req, res) => {
    try {
      ...
      let responseData = {
        filterData,
        // liftingData // comment code 17-07-25 Shubham
      }
      if (filterData) {
  ...
  static getLiftingDataViewPrint = async (req, res) => {
    try {
      ...
      if (variety_code) whereClause.variety_code = { [Op.in]: variety_code };
      if (indentor) whereClause1.id = { [Op.in]: indentor };
      if (spa) whereClause2.id = { [Op.in]: spa };
      if (bill) whereClause4.id = bill;
      ...
      while (true) {
        const prindata = await db.liftingSeedDetailsModel.findAll({
          include: [
            { model: varietyModel, attributes: [] },
            {
              model: db.agencyDetailModel3,
              required: false,
              attributes: [],
              on: {
                col1: sequelize.where(
                  sequelize.col("agency_detail.user_id"),
                  "=",
                  sequelize.col("lifting_seed_details.indentor_id")
                ),
              },
              include: [
                {
                  model: db.userModel,
                  required: false,
                  where: { user_type: "IN", ...whereClause1 },
                  attributes: [],
                },
              ],
            },
          ...
          attributes: [
            ...
            [sequelize.col("lifting_seed_details.spa_state_code"), "spa_state_code"],
            [sequelize.col("lifting_seed_details.spa_code"), "spa_code"],
            // Call the function for all SPA users per row
            [sequelize.literal(`(
                SELECT string_agg(agency_name, ', ')
                FROM get_spa_data(lifting_seed_details.spa_state_code, lifting_seed_details.spa_code)
            )`), "spa_agency_name"],
            [sequelize.literal(`(
        ...
        if (prindata.length === 0) break;
        offset += batchSize;
  
        // Group into filterMap (incrementally)
        for (const el of prindata) {
          const varietyKey = el.variety_code + (el.variety_line_code || "");
          if (!filterMap[varietyKey]) {
            filterMap[varietyKey] = {
          ...
          let indentor = filterMap[varietyKey].indentor.find((i) => i.user_id === el.user_id);
          if (!indentor) {
            indentor = { user_id: el.user_id, agency_name: el.agency_name, spa: [] };
            filterMap[varietyKey].indentor.push(indentor);
          }
      
          indentor.spa.push({
            total_price: el.total_price || "",
            // spa_agency_name:data.get("spa_agency_name")|| "",
            spa_agency_name: el.spa_agency_name || "",
            bag_weight: el.bag_weight || "",
            no_of_bag: el.no_of_bag || "",
            reason_id: el.reason_id || "",
            id: el.id || "",
            // spa_user_id: data.get("spa_user_id") || "",
            spa_user_id: el.spa_user_id || "",
            lifting_bill_no: el.lifting_bill_no || "",
            lifting_id: el.lifting_id || "",
            lotData: el.tag_no ? [{ tag_no: el.tag_no, lifting_lot_no_id: el.lifting_lot_no_id }] : [],
          });
        }
        if (prindata.length < 200) break;
      }
  
      let filterData = Object.values(filterMap);
      filterData.forEach((el) => {
        el.indentor.forEach((item) => {
          item.spa = productiohelper.removeDuplicates(item.spa, "id");
        });
      ...
      const condition = {
        ...
        include: [
          ...
          },
          {
            ...
            where: {
              ...whereClause  // This is the condition for filtering your data
            },
            attributes: []  // No attributes from the 'bspPerformaBspThree' model
          },
        ...
        attributes: [
          ...
          [sequelize.col('bsp_proforma_3->bsp_proforma_2.field_code'), 'field_code'],
          [sequelize.col('lifting_lot_number.lot_no'), 'lot_no'],
          ...
          [sequelize.col('generate_breeder_seed_certificate.refrence_sequence'), 'refrence_sequence']
          ...
          filterData.push(
            {
              ...
              "refrence_sequence": el && el.refrence_sequence ? el.refrence_sequence : '',
              "indentor":
                [
                  {
                    "user_id": el && el.user_id ? el.user_id : '',
                    ...
                    "pilot_no": el && el.field_code ? el.field_code : '',
                    "lot_no": el && el.lot_no ? el.lot_no : "",
                    "lot_id": el && el.lot_id ? el.lot_id : "",
                    "tags": [
                      {
  ...
  static savegenerateCerticate = async (req, res) => {
    ...
    try {
      ...
      let condition = {
        attributes: [
          [sequelize.col('refrence_sequence'), 'refrence_sequence'],
        ],
        order: [['refrence_sequence', 'DESC']],  // Order by descending to get the latest one
        limit: 1  // Only retrieve the latest record
      };
      ...
      try {
        ...
        let running = await db.generateBreederSeedCertificate.findAll(condition);

        if (running && running.length > 0) {
          // console.log("&&&&&&&&&&",running[0].refrence_sequence);
          // If there's a record, increment the refrence_sequence
          ref_no = running[0].refrence_sequence + 1;
        } else {
          // If no records are found, set ref_no to 1
          ref_no = 1;
        }

        console.log("Reference number:", ref_no);
      } catch (error) {
    ...
    try {
      ...
      let condition = {
        ...
        attributes: [
          ...
          [sequelize.col('variety_price_list_package.packages_size'), 'bag_size']
          //   'package_data'
        ],
        where: {
          is_active: true
    ...
    try {
      ...
      if (decreptedId) {
        let condition = {
          ...
          attributes: [
            'lifting_bill_no', 'spa_code', 'spa_state_code', 'user_id', [sequelize.col('paid_by'), 'payment_method_no'] 
            //   'package_data'
          ],
          raw: true,
        }
        ...
        for (const [index, value] of dataList.entries()) {
          let datas = await db.liftingTagNumberModel.findAll({
            where: {
              litting_seed_details_id: value.id
            },
            raw: true
          })
          ...
          console.log('datas===', groupedData);
          dataList[index]['bag_data'] = groupedData
        }
        let whereClause4 = {}
        if (decreptedId) {
        ...
        console.log("liftingCharges", liftingCharges);
  ...
  static getTagDetails = async (req, res) => {
    try {
      const { tag_no } = req.body;
      const isPreview = req.query.isPreview === 'true'; // Check the preview flag from query
      ...
      let condition = {
        ...
        include: [
          {
            ...
            required: false,
            attributes: [
              'year', 'pure_seed', 'inert_matter', 'lot_no', 'stack_no',
              'class_of_seed', 'is_status', 'no_of_bags', 'lot_qty'
            ],
            include: [
              {
                model: db.stlReportStatusModel,
                ...
                include: [
                  {
                    ...
                    attributes: ['agency_name', 'contact_person_name'],
                    include: [
                      { model: db.stateModel, attributes: ['state_code', 'state_name'] },
                      { model: db.districtModel, attributes: ['district_code', 'district_name'] },
                      { model: db.designationModel, attributes: ['name'] },
                    ]
                  }
                ]
              }
        ...
        attributes: isPreview ? ['bag_size'] : ['bag_size', 'tag_no'], // Exclude tag_no if preview mode
        nest: true,
        raw: true
      };
