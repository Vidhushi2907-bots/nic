...
class UserController {
  ...
  static getSeedTypeOfSeedInventory = async (req, res) => {
    ...
    try {
      ...
      let { variety_code, line_variety_code, year, crop_code, season } = req.body.search;
      let whereClause = {};
      if (variety_code) {
      ...
      let conditionAdditional = {
        include: [
          ...
          },
          {
            ...
            where: {
              user_id: [sequelize.col('seed_inventries.bspc_id')]
            },
            // / on: {
            //   col1: sequelize.where(sequelize.col("bsp_proforma_1.variety_line_code"), "=", sequelize.col("seed_inventries.line_variety_code")),
            // },
      ...
      let dataDirectIndent = await seedInventory.findAll(conditionAdditional);
      let bsp1Check = await db.bspPerformaBspOne.findAll({
        include: [
          {
            model: db.bspProformaOneBspc,
            attributes: []
        ...
        attributes: ['id', 'variety_code'],
        raw: true
      })


      if (bsp1Check.length < 1) {
        datas.forEach((ele, i) => {
          datas[i].isPermission = true
        })
      }
      if(dataDirectIndent.length){
        dataDirectIndent.forEach((ele, i) => {
          datas[i].isPermission = true
        })
      }
      const merged = [...datas, ...dataDirectIndent];

      const unique = Object.values(
        merged.reduce((acc, item) => {
          // Merge objects having same "value"
  ...
  static getLotNoOfSeedInventory = async (req, res) => {
    ...
    try {
      ...
      if (req.body.search) {
        ...
        if (req.body.search.exclude_tag_range && req.body.search.exclude_tag_range.length) {
        ...
        include: [
          {
            ...
            where: {
              is_active: 1,
              ...production_type,
              year: [sequelize.col('indent_of_breederseed_direct.year')],
              season: [sequelize.col('indent_of_breederseed_direct.season')],
              crop_code: [sequelize.col('indent_of_breederseed_direct.crop_code')]
            },
            attributes: []
          },
          {
  ...
  static getCarryOverVariety = async (req, res) => {
    ...
    try {
      ...
      const uniqueNationalDirectVarieties = await tempArray.filter((item, i, ar) => ar.indexOf(item) === i);
      const carryOverLines = await db.carryOverSeedModel.findAll({
      ...
      let carryOverLineArr = []
      if (carryOverLines && carryOverLines.length) {
        carryOverLines.forEach(ele => {
          if (ele.variety_line_code)
            carryOverLineArr.push(ele.variety_line_code)
        })

      }
      const directLines = await db.directIndent.findAll({
        include: [
          {
            model: db.indentOfBrseedDirectLineModel,
            attributes: [],
      ...
      console.log('directLines===', directLines);
      // return;
      let varietyCode = [];
      if (directLines && directLines.length) {
        directLines.forEach(ele => {
          if (ele.variety_code_line) {
            varietyCode.push(ele.variety_code)
          }
        })
      }
      console.log('mergedVarietyArray====', mergedVarietyArray);
      console.log('mergedVarietyArray====', varietyCode);
      let condition5 = {
        include: [
        ],
        where: {
          variety_code: {
            [Op.in]: mergedVarietyArray.length > 0 ? mergedVarietyArray : varietyCode
          },
          is_active: 1,
        },
        raw: true,
  ...
  static getClassQuantity = async (req, res) => {
    ...
    try {
      ...
      let condition = {
        ...
        attributes: [
          [sequelize.fn('SUM', sequelize.col('seed_inventries_tag.quantity_remaining')), 'quantity'],
          //   [sequelize.col('bsp_proforma_1s.variety_code'), 'variety_code'],
          [sequelize.col('seed_inventries.seed_class_id'), 'seed_class_id'],
          // [sequelize.col('seed_inventries_tag.quantity'), 'quantity'],
        ],
  ...
  static getVarietiesParentalLineForCarryOver = async (req, res) => {
    let returnResponse = {};
    const { internalCall } = req.body;
    // console.log('internalCall===',internalCall)
    try {
      let rules = {
        'search.year': 'string',
        'search.season': 'string',
      ...
      if (req.body.search.variety_code && req.body.search.variety_code) {
  ...
  static getSeedProcessingRegData = async (req, res) => {
    try {
      ...
      const lifitingData = await liftingSeedDetailsModel.findAll({
        ...
        raw: true,
        attributes: [
          'spa_code', 'spa_state_code', 'indentor_id', 'variety_code'
        ]
      })
      // Extract all variety codes
      const varietyCodes = lifitingData.map(item => item.variety_code);

      const varierydata = await cropVerietyModel.findAll({
        where: {
      ...
      console.log("varieryid", varierydata);
      const varietyID = varierydata.map(item => item.id);
      ...
      if (lifitingData && lifitingData.length > 0) {
        for (let key in lifitingData) {
          ...
          const receiptRequestData2 = await liftingSeedDetailsModel.findAll({
            include: [
              ...
              },
              {
                ...
                where: {
                  ...
                  user_id: lifitingData[key].indentor_id,
                },
                include: [
                  {
                    model: allocationSpaForLiftingSeed,
                    as: 'allocationSpaForLiftingSeed',
                    required: false,
              ...
              },
              {
                ...
                where: {
                  ...
                  season: season,
                  crop_code: crop_code,
                  variety_code: lifitingData[key].variety_code,
                  indenter_id: lifitingData[key].indentor_id,
                  spa_code: lifitingData[key].spa_code,
                  state_code: lifitingData[key].spa_state_code
                }
              },
            ...
            attributes: [
              'id',
              [db.Sequelize.fn('to_char', db.Sequelize.col('lifting_seed_details.created_at'), 'DD-MM-YYYY'), 'date_of_lifting']
            ],
            raw: true
          });
      ...
      receiptRequestData.forEach(item => {
        ...
        if (!variety) {
          variety = {
            id: item.id,
            variety_id: item['m_crop_variety.id'],
        ...
        console.log("item", item['allocation_to_spa_for_lifting_seed.allocationSpaForLiftingSeed.']);
        if (!spaDetail) {
          spaDetail = {
            spa_code: item['userModelSpa.spa_code'],
            spa_name: item['userModelSpa.name'],
            indent_qty: item['indenterSPAModell.indent_quantity'],
            qty_of_breeder_seed: item['allocation_to_spa_for_lifting_seed.allocationSpaForLiftingSeed.'] || 0,
        ...
        const bagDetail = {
          tag_size: item['lifting_tag_number.tag_size'],
          no_of_bags: item['lifting_tag_number.no_of_bags']
        };
        // console.log(bagDetail,"jhfgfg");
  ...
  // new api for update bag size
  static updateBagSizeData = async (req, res) => {
    const { sequelize } = db; // Make sure db exports sequelize instance
    const transaction = await sequelize.transaction();
    try {
      const { id, is_edit, bag_data, total_qnt, calculated_qnt, remaining_qnt } = req.body;

      // Step 1: Fetch all original data
      const existDataSpr = await db.seedProcessingRegister.findAll({
        where: { id },
        raw: true,
        transaction: transaction
      });

      const existDataSprStack = await db.SeedForProcessedStack.findAll({
        where: { seed_processing_register_id: id },
        raw: true,
        transaction: transaction
      });

      const existDataProcess = await db.ProcessSeedDetails.findAll({
        where: { seed_processing_register_id: id },
        raw: true,
        transaction: transaction
      });
      let newSeedProcessingRegisterId;
      // Step 2: Move data to backup tables
      for (const record of existDataSpr) {
        const { lot_no, variety_code, year, user_id } = record;

        // Deactivate existing backups
        await db.seedProcessingRegisterBkpsModel.update(
          { is_active: 0 },
          {
            where: {
              lot_no, variety_code, year, user_id,
              is_active: 1
            },
            transaction: transaction
          }
        ...
        const backupData = await db.seedProcessingRegisterBkpsModel.create(
          { ...rest, is_active: 1, is_edit },
          { transaction: transaction }
        );
        // 3. Use this new ID wherever needed
        newSeedProcessingRegisterId = backupData.id;
      }
      console.log(newSeedProcessingRegisterId);
      if (existDataSprStack.length) {
        const stackData = existDataSprStack.map(({ id, seed_processing_register_id, ...item }) => ({
          ...item,
          seed_processing_register_id: newSeedProcessingRegisterId,
          is_active: 1,
          is_edit
        }));
        await db.seedForProcessedStackBkpsModel.bulkCreate(stackData, { transaction: transaction });
      }

      if (existDataProcess.length) {
        const procData = existDataProcess.map(({ id, seed_processing_register_id, ...item }) => ({
          ...item,
          seed_processing_register_id: newSeedProcessingRegisterId,
        }));
        await db.processedSeedDetailsBkpsModel.bulkCreate(procData, { transaction: transaction });
      }

      // Step 3: If edit, then update original table
      if (is_edit === 1 && Array.isArray(bag_data) && bag_data.length) {
        const firstBag = bag_data[0];
        let noOfBag = 0;
        // 2. Delete old rows
        await db.ProcessSeedDetails.destroy({
          where: { seed_processing_register_id: id },
          transaction: transaction,
        });

        await db.SeedForProcessedStack.destroy({
          where: { seed_processing_register_id: id },
          transaction: transaction,
        });

        // 3. Insert new rows for each bag_data item
        for (let i = 0; i < bag_data.length; i++) {
          const { no_of_bags, bags } = bag_data[i];
          console.log('no_of_bags=====', no_of_bags);
          noOfBag += parseInt(no_of_bags);
          const qty = no_of_bags * bags;
          const bagSequence = Array.from({ length: no_of_bags }, (_, idx) => idx + 1).join(',');

          // 3a. Create new ProcessSeedDetails row
          await db.ProcessSeedDetails.create(
            {
              seed_processing_register_id: id,
          ...
          // 3b. Create new SeedForProcessedStack row
          await db.SeedForProcessedStack.create(
            {
              seed_processing_register_id: id,
              no_of_bag: bagSequence,
              type_of_seed: existDataSprStack && existDataSprStack.type_of_seed || 1, // adjust fallback as per schema
              stack_no: existDataSprStack && existDataSprStack.stack_no || 'K/24-25/PS/3',
              godown_no: existDataSprStack && existDataSprStack.godown_no || 1,
            },
            { transaction: transaction }
          );
        }
        // 1. Update seedProcessingRegister
        await db.seedProcessingRegister.update(
          {
            no_of_bags: noOfBag,
            total_processed_qty: calculated_qnt,
            recover_qty: total_qnt,
            remaining_qty: remaining_qnt,
            is_edit: 1,
          },
          {
            where: { id },
      ...
      return response(res, status.SERVER_ERROR, 500, { message: 'Something went wrong', error });
    }
  }

  // new api for bspc lifting variety and production count

  static getVarietycount = async (req, res) => {
    try {
      const { year, season, crop_code, user_id } = req.body.search;

      let condition = {
        attributes: [
          [sequelize.fn('COUNT', sequelize.fn('DISTINCT', sequelize.col('variety_code'))), 'total_varieties']
        ],
        where: {
          year,
          season,
      ...
      let dataList = await db.seedTagDetails.findOne(condition); // use findOne, not findAll
      console.log("dataList", dataList);

      if (dataList && dataList.total_varieties) {
        response(res, status.DATA_AVAILABLE, 200, dataList);
      } else {
        response(res, status.DATA_NOT_AVAILABLE, 201, { total_varieties: 0 });
      }
    } catch (error) {
      console.log(error);
  ...
  static gettotalproduction = async (req, res) => {
    try {
      const { year, season, crop_code, user_id } = req.body.search;
      ...
      let condition = {
        include: [
          {
            ...
            as: "seed_tag",   // alias must match here
            attributes: [],
            where: { is_active: 0 }
          }
        ],
        attributes: [
          [
            sequelize.fn(
              "SUM",
              sequelize.literal('"seed_tag"."no_of_bags" * "seed_tag"."bag_size"')
            ),
            "total_production"
          ]
        ],
      ...
      let dataList = await db.seedTagDetails.findOne(condition);
      console.log("totalproduction is", dataList);

      if (dataList && dataList.total_production) {
        response(res, status.DATA_AVAILABLE, 200, dataList);
      } else {
        response(res, status.DATA_NOT_AVAILABLE, 201, { total_production: 0 });
      }
    } catch (error) {
      console.log(error);
      response(res, status.UNEXPECTED_ERROR, 501, []);
  ...
  static gettotallifting = async (req, res) => {
    try {
      const { year, season, crop_code, user_id } = req.body.search;
      ...
      let condition = {
        attributes: [
          [
            sequelize.fn(
              "SUM",
              sequelize.literal('"no_of_bag" * "bag_weight"') // make sure column name is correct
            ),
            "total_lifting"
          ]
        ],
      ...
      let dataList = await db.liftingSeedDetailsModel.findOne(condition);
      console.log("total_lifting is", dataList);

      if (dataList && dataList.total_lifting) {
        response(res, status.DATA_AVAILABLE, 200, dataList);
      } else {
        response(res, status.DATA_NOT_AVAILABLE, 201, { total_lifting: 0 });
      }
    } catch (error) {
      console.log(error);
      response(res, status.UNEXPECTED_ERROR, 501, []);
