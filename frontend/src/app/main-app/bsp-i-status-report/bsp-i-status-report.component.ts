...
@Component({
  selector: 'app-bsp-i-status-report',
  templateUrl: './bsp-i-status-report.component.html',
  styleUrls: ['./bsp-i-status-report.component.css']
})
export class BspIStatusReportComponent implements OnInit {
  @ViewChild(PaginationUiComponent) paginationUiComponent!: PaginationUiComponent;
  @ViewChild(BspProfarmaOneFormComponent, { static: false }) bspProfarmaOneFormComponent!: BspProfarmaOneFormComponent;
  ...
  ngOnInit(): void {
    ...
    console.log('Child component:', this.bspProfarmaOneFormComponent); 
  ...
  BspCropData(newData) {
    let route = "get-bsp-proforma-one-crop-data-report";
    let param = {
      "search": {
        "year": this.ngForm.controls['year'].value,
  ...
  getPageData(loadPageNumberData: number = 1, searchData: any | undefined = undefined) {
    ...
    this.productioncenter.postRequestCreator("bsp-one-status-reports", filters, null)
      .subscribe((apiResponse: any) => {
        if (apiResponse && apiResponse.EncryptedResponse.status_code === 200) {
          this.allData = apiResponse.EncryptedResponse.data || [];
          console.log("this.allData",this.allData); 
          console.log("agency_id====",this.allData.agency_id);
  ...
  downloadPdf(userId,agency_id) {
    console.log('userId========',userId);
      ...
      // this.bspProfarmaOneFormComponent.ngOnInit();
      // Call the child component's generatePdf method
      
      this.bspProfarmaOneFormComponent.getAgencyData(null,param1); 
      this.bspProfarmaOneFormComponent.getCropData(param); 
      this.bspProfarmaOneFormComponent.getPageData(null,null,param); 
      this.bspProfarmaOneFormComponent.getBspcTeamData(null,null,param);
      // this.bspProfarmaOneFormComponent.generatePdf(param);
  }
  toggleSearch() {
