...
@Component({
  selector: 'app-discard-lot',
  templateUrl: './discard-lot.component.html',
  styleUrls: ['./discard-lot.component.css']
})
export class DiscardLotComponent implements OnInit {
  ...
  getReprintVarietyList() {
    let route = "get-discard-tag-variety";
    let param;
    this._productionCenter.postRequestCreator(route, param, null).subscribe(res => {
      if (res.EncryptedResponse.status_code === 200) {
  ...
  getReprintAllData() {
    let route = "get-discard-tag-data";
    let param = {
      "year": this.ngForm.controls['year'].value,
      "season": this.ngForm.controls['season'].value,
      "crop_code": this.ngForm.controls['crop_code'].value,
  ...
  clear() {
    ...
    this.ngForm.controls['user_agreement'].setValue('');
  ...
  submit(formValue) {
    ...
    let route = 'add-disacrd-tag-data';
    let variety;
    let lotNo;
    ...
    if (this.ngForm.controls['id'].value) {
      ...
      message = "Discard Update Tag"
    } else {
      message = "Discard Saved Tag"
    }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this._productionCenter.postRequestCreator(route, { 'reprintTagData': param }, null).subscribe(res => {
          if (res.EncryptedResponse.status_code === 200) {
            Swal.fire({
              title: '<p style="font-size:25px;">Selected tag(s) ' + message + 'successfully.</p>',
              icon: 'success',
              confirmButtonText:
                'OK',
              confirmButtonColor: '#E97E15'
  ...
  edit(data) {
    ...
    let tagArrayData = [];
    if (data) {
      data.discard_tag_request.forEach(el => {
        if (el.tag_no) {
          tagArrayData.push({ "tag_no": el.tag_no })
        }
      })
    }

    if (data) {
      this.ngForm.controls['id'].patchValue(data && data.reprint_id ? data.reprint_id : null);
      this.ngForm.controls['variety'].patchValue(data && data.variety_code ? data.variety_code : '');
      this.ngForm.controls['parental_line'].patchValue(data && data.variety_line_code ? data.variety_line_code : '');
      this.ngForm.controls['lot_no'].patchValue(data && data.lot_no ? data.lot_no : '');
      this.ngForm.controls['lot_id'].patchValue(data && data.lot_id ? data.lot_id : '');
      this.ngForm.controls['reason_for_reprint'].patchValue(data && data.reason ? data.reason : null);
      this.ngForm.controls['no_of_tags'].patchValue(tagArrayData.length);
      this.ngForm.controls['tag_for_reprint'].patchValue(tagArrayData.length ? tagArrayData : null);
    }
  }
  ...
  delete(id) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let route = 'delete-discard-tag-data';
        this._productionCenter.postRequestCreator(route, { 'id': id }, null).subscribe(res => {
          if (res.EncryptedResponse.status_code === 200) {
            Swal.fire({
              title: '<p style="font-size:25px;">data deleted successfully.</p>',
              icon: 'success',
              confirmButtonText:
                'OK',
              confirmButtonColor: '#E97E15'
