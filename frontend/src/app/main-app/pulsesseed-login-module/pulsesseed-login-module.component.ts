...
@Component({
  selector: 'app-pulsesseed-login-module',
  templateUrl: './pulsesseed-login-module.component.html',
  styleUrls: ['./pulsesseed-login-module.component.css']
})
export class PulsesseedLoginModuleComponent implements OnInit {
          ...
          if (apiResponse && apiResponse.EncryptedResponse && apiResponse.EncryptedResponse.data && apiResponse.EncryptedResponse.data.data.length > 0) {
            ...
            if (this.searchValue) {
              const searchFilters = {
                ...
                "group_code": this.response_crop_group && this.response_crop_group[0] && this.response_crop_group[0]['m_crop_group.group_code'] ? this.response_crop_group[0]['m_crop_group.group_code'] : 'A03',
  ...
  Submit(formData) {
    ...
    if (formData && formData.valid) {
      ...
      const groupName = this.response_crop_group.filter(x => {
        return x['m_crop_group.group_code'] == 'A03';
      });
