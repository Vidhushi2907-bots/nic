...
@Component({
  selector: 'app-state-wise-crop-statistics',
  templateUrl: './state-wise-crop-statistics.component.html',
  styleUrls: ['./state-wise-crop-statistics.component.css']
})
export class StateWiseCropStatisticsComponent implements OnInit {
  @Output() reportSwitched = new EventEmitter<string>();
  ngFrom: FormGroup;
  currentLevel: 'state' | 'group' | 'crop' | 'variety' | 'final' = 'state';
  ...
  stateColumns = [
    { name: "SN", isIndex: true, width: 10 },
    { name: "State Name", dbColumnName: "state_name", width: 25 },
    { name: "Total Group Count", dbColumnName: "total_crop_group_count", width: 25 },
    { name: "Total Crop Count", dbColumnName: "total_crop_count", width: 20 },
    { name: "Total Variety Count", dbColumnName: "total_variety_count", width: 20 },
  ];
  cropColumns = [
 ...
 createForm() {
    ...
    this.ngFrom.controls["group_code_search_filter"].valueChanges.subscribe(newvalue => {
      this.getCropGroupWiseList(this.state_code);
    });
    this.ngFrom.controls["crop_search_filter"].valueChanges.subscribe(newvalue => {
      this.getCropWiseList(this.state_code, this.group_code);
    });
    this.ngFrom.controls["variety_search_filter"].valueChanges.subscribe(newvalue => {
      this.getVarietyWiseList(this.state_code, this.group_code,this.crop_code);
    });
 }
 ...
 goBack(): void {
    ...
    // else if (this.currentLevel === 'group') this.currentLevel = 'state';
 }
 ...
 getStateWiseList() {
  ...
  this.service.postRequestCreator('get-state-data-state-wise-report-two', null, payload)
    .subscribe(
      (res: any) => {
        const data = res?.EncryptedResponse?.data;
        if (res?.EncryptedResponse?.status_code === 200 ) {
 ...
 getCropGroupWiseList(state_code: any) {
  ...
  this.service.postRequestCreator('get-state-data-crop-group-wise-report-two', null, payload)
    .subscribe(
      (res: any) => {
        const data = res?.EncryptedResponse?.data;
        if (res?.EncryptedResponse?.status_code === 200 ) {
 ...
 getCropWiseList(state_code: any,group_code: any) {
  this.state_code= state_code;
 ...
 getVarietyWiseList(state_code: any, group_code: any, crop_code: any) {
  this.currentLevel = 'variety';
  this.state_code= state_code;
  this.group_code= group_code;
 ...
 sortData(pageType: any, column: string) {
    let data: any;
    if(pageType == 'state') data= this.stateWiseDataList;
    else if(pageType == 'group') data= this.cropGroupWiseDataList;
