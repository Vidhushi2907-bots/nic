...
@Component({
  selector: 'app-variety-report-list',
  templateUrl: './variety-report-list.component.html',
  styleUrls: ['./variety-report-list.component.css']
})
export class VarietyReportListComponent implements OnInit, AfterViewInit {
  // VarietyCharactersticViewFormComponent
//  @ViewChild(VarietyReportListViewComponent) VarietyReportListViewComponent:  VarietyReportListViewComponent| undefined = undefined;

 @ViewChild(VarietyReportListViewComponent)
 VarietyReportListViewComponent!: VarietyReportListViewComponent;
  ...
  ngAfterViewInit(): void {
   ...
   getCropNotDateData() {
     this.service.postRequestCreator("get-no-year-character-data", null, null).subscribe((apiResponse: any) => {
         if (apiResponse && apiResponse.EncryptedResponse && apiResponse.EncryptedResponse.status_code
           ...
           // this.cropNotDateData.sort((a, b) => (a && a.notification_date).localeCompare(b && b.notification_date && b.notification_date))
   ...
   updatePaginatedData(): void {
     ...
     this.VarietyReportListViewComponent.getListData(this.paginatedData[0],null,this.loadpageNo);
   }
   ...
   nextPage() {
     // if (this.currentPage * this.pageSize < this.allData.length) {
       ...
       this.VarietyReportListViewComponent.nextPage();
     // }
   }
   ...
   previousPage() {
     // if (this.currentPage > 1) {
       this.currentPage--;
       // this.updatePaginatedData();
       this.VarietyReportListViewComponent.previousPage();
     // }
   }
     ...
     if(!this.disableFiledsByYear){
       ...
       if (this.disableFileds) {
         if ((!this.ngForm.controls["crop_group"].value && !this.ngForm.controls["crop_name"].value && !this.ngForm.controls["variety_name"].value)) {
           ...
           this.VarietyReportListViewComponent.search(paramData);
  ...
  //    console.log("--this.VarietyReportListViewComponent-", this.VarietyReportListViewComponent)
  //    this.VarietyReportListViewComponent.getListData(data,dataIndex,this.loadpageNo);
  //  }

   patchData(data: any, dataIndex: number): void {
    this.hideViewSection = true;
  
    setTimeout(() => {
      if (this.VarietyReportListViewComponent) {
        this.VarietyReportListViewComponent.getListData(data, dataIndex, this.loadpageNo);
      }
    });
  }
